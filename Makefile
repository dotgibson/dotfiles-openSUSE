...
# The exact two header lines upstream's sync-core.sh writes into core.lock. Reproduced
# byte-for-byte so `make core-lock` is IDEMPOTENT: regenerating an unchanged lock must
# produce no diff. Emitting our own wording instead made every regeneration look like
# drift and would fight the next fan-out, which rewrites the file with these lines.
LOCK_HEADER_1 := \# GENERATED by dotfiles-core sync-core.sh — vendored Core provenance (B1).
LOCK_HEADER_2 := \# Regenerate after a manual 'git subtree pull' with: make core-lock
